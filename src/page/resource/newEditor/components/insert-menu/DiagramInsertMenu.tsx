...
function DiagramInsertMenu(props: DiagramInsertMenuProps) {
  const { disabled, editor } = props;
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const label = t('resource.editor.toolbar.diagram');
    ...
    <Popover open={!disabled && open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" side="bottom" className="w-40 p-1">
        <button
          ...
          onClick={() => insertSnippet(createMermaidSnippet())}
        >
          {t('resource.editor.insert.mermaid')}
        </button>
        <button
          type="button"
          className={cn(
          ...
          onClick={() => insertSnippet(createEchartsSnippet())}
        >
          {t('resource.editor.insert.echarts')}
        </button>
      </PopoverContent>
    </Popover>
  );
...
export default DiagramInsertMenu;
