...
interface RssFolderUnsavedDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm: () => void;
}

export function RssFolderUnsavedDialog(props: RssFolderUnsavedDialogProps) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-[560px]">
        <AlertDialogHeader>
          <AlertDialogTitle>
            {t('rss_folder.create.unsaved_title')}
          </AlertDialogTitle>
          <AlertDescription className="text-muted-foreground text-sm">
            {t('rss_folder.create.unsaved_description')}
          </AlertDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
