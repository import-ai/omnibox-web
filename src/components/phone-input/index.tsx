...
function PhoneNumberInput({
  ...
  return (
    <div
      className={cn(
        'border-line flex h-10 w-full items-center rounded-md border bg-transparent transition-[color,box-shadow] focus-within:outline-none focus-within:ring-1 focus-within:ring-ring',
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild disabled={disabled}>
            <Button
              type="button"
              variant="ghost"
              className="border-line h-full shrink-0 gap-1 rounded-l-md rounded-r-none border-r px-2 text-sm hover:bg-accent focus-visible:ring-0"
            >
              <span className="text-base">{selectedCountryData.flag}</span>
              <span className="text-muted-foreground">
                {selectedCountryData.dialCode}
